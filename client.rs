...
async fn get_address(owner: &str) -> Wallet {
	for node in 0..N_NODES {
		match get(format!("{}/api/wallet/{}", get_node_enpoint(node), owner)).await {
			Ok(response) => {
...
async fn get_priv_key(token_id: String, owner: String, verifier: String) -> String {
	...
	let threshold_pub_key =
		threshold_same(shares.iter().map(|share| share.pub_key.clone()).collect(), THRESHOLD)
			.unwrap();
